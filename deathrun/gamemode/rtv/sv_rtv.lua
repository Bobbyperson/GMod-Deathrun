...
if RTV.UsePrefixes then
    for k, v in RandomPairs(files) do
        if get >= RTV.Limit then break end --RTV.Maps[#RTV.Maps+1] = "Extend Current Map"
...
function RTV.CanVote(ply)
    ...
    if ply.RTVoted then return false, "You have already voted to Rock the Vote!" end
