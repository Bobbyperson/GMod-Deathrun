...
if SERVER then
    ...
    hook.Add("ChangePlayerModel", "Overwrite Player Model Example", function(ply)
        if ply:IsUserGroup("vip") then return "models/player/leet.mdl" end -- If the player is in the "vip" group -- Set their model to the CS:S leet model.
    end)
    -- Stop the server from going any further.
...
hook.Add("GetScoreboardNameColor", "Name Color Example", function(ply)
    if ply:IsUserGroup("vip") then return Color(255, 0, 0, 255) end -- If the player is in the "vip" group -- give the player a red name.
end)

hook.Add("GetScoreboardIcon", "Icon Example", function(ply)
    if ply:IsUserGroup("vip") then return "icon16/heart.png" end -- If the player is in the "vip" group -- give the player a heart icon! You can find 16x16 silkicons inside garrysmod/materials/icon16/
end)
