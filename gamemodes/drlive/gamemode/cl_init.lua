...
language.Add("func_movelinear", killerName)
language.Add("func_physbox", killerName)
language.Add("func_rotating", killerName)
language.Add("func_door", killerName)
language.Add("entityflame", killerName)
language.Add("prop_physics", killerName)
...
net.Receive("_KeyRelease", function()
    ...
    if not IsValid(ply) then
        print("Invalid keyrelease player.")
...
function GM:CreateMove(uc)
    if GetGlobalInt("dr_allow_autojump") == 1 and GetConVar("deathrun_autojump"):GetInt() == 1 and LocalPlayer():WaterLevel() < 3 and LocalPlayer():Alive() and LocalPlayer():GetMoveType() == MOVETYPE_WALK and not LocalPlayer():InVehicle() and bit.band(uc:GetButtons(), IN_JUMP) > 0 then
        if LocalPlayer():IsOnGround() then
            uc:SetButtons(uc:GetButtons() or IN_JUMP)
        else
            uc:SetButtons(bit.band(uc:GetButtons(), bhstop))
        end
    end
end
...
function GM:GetScoreboardNameColor(ply)
    ...
    if GetGlobalInt("dr_highlight_admins") == 1 and ply:IsAdmin() then return Color(220, 180, 0, 255) end
end
...
function GM:GetScoreboardIcon(ply)
    ...
    if GetGlobalInt("dr_highlight_admins") == 1 and ply:IsAdmin() then return "icon16/shield.png" end
end

local function GetIcon(str)
    if str == "1" then return "icon16/tick.png" end
...
-- Credit goes to BKU for this function!
function WrapText(text, width, font)
    ...
    -- Nope, but wrap in table for uniformity
...
local function ShowHelp()
    ...
    dlist:SetPos(5, 30)
    dlist:EnableVerticalScrollbar(true)
    local fullText = string.Explode("\n", GAMEMODE.MenuText)

    for k, v in pairs(fullText) do
