...
-- cvars.AddChangeCallback doesn't seem to want to work; no idea why. 
...
function GM:PlayerCanPickupWeapon(ply, wep)
    ...
    if ply:Team() == TEAM_DEATH and wep:GetClass() ~= "weapon_crowbar" and pickupConvar:GetInt() == 0 then return false end
...
function GM:GetFallDamage(ply, speed)
    if falldamage:GetInt() == 1 then return speed / 8 end
...
local function NextPlayer(ply)
    ...
    if not IsValid(new) then return plys[1] end
...
local function PrevPlayer(ply)
    ...
    for k, v in pairs(plys) do
        if v == ply then return old or plys[#plys] end
        old = v
    end

    if not IsValid(old) then return plys[#plys] end
...
function GM:KeyPress(ply, key)
    ...
    if SpecFuncs[key] then
        if not ply._unspec_deathrag then return SpecFuncs[key](ply) end
